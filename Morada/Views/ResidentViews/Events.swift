...
struct Events: View {
    @ObservedObject var moreViewModel = EventsViewModel()
                ...
                .toolbar(content: {
                    ToolbarItem(placement: .principal) {
                            CustomToolbar(
                                ...
                                AddEvent(viewModel: moreViewModel,isPresented: $isSheetPresented).presentationDetents([.fraction(0.9)]).onDisappear{
                                    Task{
                                        await moreViewModel.getEvents()
